...
export const alertsApi = {
  getAll: async (unresolved?: boolean) => {
    const params = unresolved !== undefined ? `?unresolved=${unresolved}` : '?limit=100';
...
export const emailsApi = {
  ...
  fetchAndAnalyze: async (days?: number, limit?: number) => {
    const params = new URLSearchParams();
    if (days) params.append('days', days.toString());
    if (limit) params.append('limit', limit.toString());
    const query = params.toString() ? `?${params.toString()}` : '';
    const response = await api.post(`/emails/fetch-and-analyze${query}`);
    return response.data;
  },
  ...
  getBackupSummary: async (days?: number) => {
    const query = days ? `?days=${days}` : '';
    const response = await api.get(`/emails/backup-summary${query}`);
    return response.data;
  },
  getAnalyzedEmails: async (page?: number, limit?: number, status?: string) => {
    const params = new URLSearchParams();
    if (page) params.append('page', page.toString());
    if (limit) params.append('limit', limit.toString());
    if (status) params.append('status', status);
    const query = params.toString() ? `?${params.toString()}` : '';
    const response = await api.get(`/emails/analyzed${query}`);
    return response.data;
  },
