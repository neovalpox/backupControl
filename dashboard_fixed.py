...
async def get_dashboard_summary(
    ...
    backups_failed = status_counts.get("failed", 0) + status_counts.get("critical", 0)
    backups_warning = status_counts.get("warning", 0) + status_counts.get("alert", 0)
    backups_unknown = status_counts.get("unknown", 0) + status_counts.get(None, 0)
    ...
    success_rate = round(backups_ok / total_backups * 100 if total_backups > 0 else 0, 1)

    # Tendance simplifiee (comparer avec semaine derniere)
    week_ago = datetime.utcnow() - timedelta(days=7)
    two_weeks_ago = datetime.utcnow() - timedelta(days=14)

    result = await db.execute(
        select(func.count(BackupEvent.id))
        .where(
            BackupEvent.event_date >= week_ago,
            BackupEvent.event_type == "success"
        )
    )
    success_this_week = result.scalar() or 0

    result = await db.execute(
        select(func.count(BackupEvent.id))
        .where(BackupEvent.event_date >= week_ago)
    )
    total_this_week = result.scalar() or 0

    result = await db.execute(
        select(func.count(BackupEvent.id))
        .where(
            BackupEvent.event_date >= two_weeks_ago,
            BackupEvent.event_date < week_ago,
            BackupEvent.event_type == "success"
        )
    )
    success_last_week = result.scalar() or 0
    ...
    total_last_week = result.scalar() or 0

    rate_this_week = success_this_week / total_this_week * 100 if total_this_week > 0 else 0
    rate_last_week = success_last_week / total_last_week * 100 if total_last_week > 0 else 0
    success_rate_trend = round(rate_this_week - rate_last_week, 1)

    return {
        "total_clients": total_clients,
        "total_backups": total_backups,
        "backups_success": backups_ok,
        "backups_failed": backups_failed,
        "backups_warning": backups_warning,
        "backups_unknown": backups_unknown,
...
async def get_recent_events(
    ...
    """Derniers evenements de sauvegarde"""
    result = await db.execute(
        select(BackupEvent, Backup.name.label("backup_name"), Client.name.label("client_name"))
        .select_from(BackupEvent)
        .join(Backup, BackupEvent.backup_id == Backup.id)
        .join(Client, Backup.client_id == Client.id)
...
async def acknowledge_alert(
    ...
    return {"message": "Alerte acquittee"}
