...
// Types generiques pour eviter les erreurs TypeScript
...
export const setApiToken = (_token: string | null) => {
  console.log('setApiToken called (no-op)');
};
...
export const backupsApi = {
  ...
  getEvents: async (clientId?: number) => {
    const params = clientId ? { client_id: clientId } : {};
    const response = await api.get('/backups/events/', { params });
    return response.data;
  },
  setMaintenance: async (id: number, inMaintenance: boolean) => {
    const response = await api.patch(`/backups/${id}/maintenance`, { in_maintenance: inMaintenance });
    return response.data;
  },
};
...
export const emailsApi = {
  getAll: async (backupOnly = false, limit = 100, offset = 0) => {
  ...
  fetchNew: async (limit = 100) => {
    const response = await api.post('/emails/fetch', { limit });
    return response.data;
  },
  analyzeAll: async () => {
  ...
  fetchAndAnalyze: async (limit = 500, analyze = true) => {
    const response = await api.post('/emails-v2/fetch-and-analyze', {
      limit,
      folder: 'INBOX',
      analyze
    }, {
  ...
  getAnalysisProgress: async () => {
    const response = await api.get('/emails-v2/analysis/progress');
    return response.data;
  },
  getBackupSummary: async (days = 30) => {
    const response = await api.get(`/emails-v2/backup-summary?days=${days}`);
    return response.data;
  },
  getAnalyzedEmails: async (backupOnly = false, limit = 200, offset = 0) => {
    const response = await api.get('/emails-v2/analyzed', {
      params: { backup_only: backupOnly, limit, offset }
    });
    return response.data;
  },
  getAnalyzedEmailDetail: async (emailId: number) => {
    const response = await api.get(`/emails-v2/analyzed/${emailId}`);
    return response.data;
  },
  getSettings: async () => {
