...
export default function Backups() {
  ...
  const toggleMaintenance = async (backup: Backup) => {
    try {
      await backupsApi.setMaintenance(backup.id, !backup.maintenance_mode);
      fetchBackups();
    } catch (error) {
                ...
                filteredBackups.map((backup) => (
                  <tr
                    ...
                    className={`hover:bg-gray-700/50 cursor-pointer ${
                      backup.maintenance_mode ? 'opacity-50' : ''
                    }`}
                    onClick={() => handleBackupClick(backup)}
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className={`p-2 rounded-lg mr-3 ${getStatusBgColor(backup.last_status || 'unknown')}`}>
                          {getStatusIcon(backup.last_status || 'unknown')}
                        </div>
                        <div>
                          <p className="text-white font-medium">{backup.name}</p>
                          {backup.maintenance_mode && (
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        backup.last_status === 'success'
                          ...
                          : backup.last_status === 'warning'
                          ? 'bg-yellow-500/10 text-yellow-500'
                          : 'bg-gray-500/10 text-gray-500'
                      }`}>
                        {t(`status.${backup.last_status || 'unknown'}`)}
                      </span>
                    </td>
          ...
          <div className="relative bg-gray-800 rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center">
                  {getStatusIcon(selectedBackup.last_status || 'unknown', 'w-4 h-4')}
                  <span className="ml-2 text-white">{t(`status.${selectedBackup.last_status || 'unknown'}`)}</span>
                </div>
              </div>
              <div>
                <p className="text-gray-500 text-sm">{t('backups.lastRun')}</p>
                <p className="text-white">{formatDate(selectedBackup.last_run)}</p>
              </div>
              <div>
                <p className="text-gray-500 text-sm">{t('backups.size')}</p>
                <p className="text-white">{formatSize(selectedBackup.last_size)}</p>
              </div>
            </div>
